 ...
 */
async function decodeLnurlPay(lnurl: string): Promise<DecodedLnurlPayRequest> {
  try {
    ...
    throw new Error(`Failed to decode LNURL-pay: ${e.message}`);
  }
}
 ...
 */
function parseLightningInvoice(invoiceStr: unknown): ParsedLightningInvoice {
 ...
 */
function validateLnurlInvoice(invoice: ParsedLightningInvoice, millisatAmount: bigint, metadata: string): void {
 ...
 */
async function fetchLnurlPayInvoice(params: LnurlPayParams): Promise<LnurlPayInvoiceResponse> {
  try {
    ...
    throw new Error(`Failed to fetch LNURL payment invoice: ${e.message}`);
  }
}
 ...
 */
async function processLnurlPayment(
  lnurl: string,
  amountSats: bigint,
  comment?: string
): Promise<{ invoice: string; description?: string; successAction?: unknown }> {
  try {
    ...
    // Parse metadata
    const { description } = parsePayMetadata(payResponse.metadata);
    ...
    throw new Error(`LNURL payment processing failed: ${e.message}`);
  }
}
...
function decodeLnurl(lnurl: string): string {
  ...
  try {
    ...
    throw new Error(`invalid lnurl: ${e.message}`);
  }
}
 ...
 */
function isValidLnurl(str: string): boolean {
 ...
 */
function parsePayMetadata(metadataStr: string): { description?: string } {
  try {
    ...
    const textMetadata = parsed.find((item) => item?.mime === 'text/plain');
    return { description: textMetadata?.text };
  } catch (e) {
    throw new Error(`Failed to parse LNURL metadata: ${e.message}`);
  }
}
