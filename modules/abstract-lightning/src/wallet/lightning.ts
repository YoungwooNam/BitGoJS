...
export class LightningWallet implements ILightningWallet {
  ...
  private async payInvoice(
    params: t.TypeOf<typeof SubmitPaymentParams> & { invoice: string }
   ...
   * @private
   */
  private async payLnurl(
    params: t.TypeOf<typeof SubmitPaymentParams> & { lnurl: string }
  ): Promise<PayInvoiceResponse> {
    try {
      ...
      // Create a new params object with the invoice instead of lnurl
      return this.payInvoice({
        ...
        lnurl: undefined as any, // Remove lnurl property
      });
    } catch (error: any) {
